...
import { createHighlight, deleteHighlight, getSession, listAllHighlights, listHighlights, signIn, signOut, supabaseConfigured, updateHighlight, uploadFile, listCupResults, createCupResult, deleteCupResult } from "./supabase";
...
export function Admin() {
  ...
  async function refresh() { if (!session) return; try { const [highlights, cups] = await Promise.all([listAllHighlights(session), listCupResults()]); setItems(highlights); setCupResults(cups); } catch(e) { setMessage(e.message); } }
