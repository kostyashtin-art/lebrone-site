...
import { createHighlight, deleteHighlight, getSession, listAllHighlights, listHighlights, signIn, signOut, supabaseConfigured, updateHighlight, uploadFile } from "./supabase";
...
export function Admin() {
  ...
  async function refresh() { if (!session) return; try { setItems(await listAllHighlights(session)); } catch(e) { setMessage(e.message); } }
  ...
  if (!supabaseConfigured) return <PageShell title="ADMIN" sub="ПАНЕЛЬ УПРАВЛЕНИЯ ХАЙЛАЙТАМИ"><div className="admin-setup"><h2>Нужно подключить Supabase</h2><p>В проект добавлены готовые загрузка MP4, авторизация, база хайлайтов и Storage. Осталось указать два публичных параметра проекта в .env.</p><code>VITE_SUPABASE_URL=…<br/>VITE_SUPABASE_ANON_KEY=…</code><p>Секретный service-role key на сайт не добавляем.</p></div></PageShell>;
