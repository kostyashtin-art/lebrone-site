...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.startswith("/api/d2pt/hero/") and path.endswith("/pro-builds"):
            parts = path.strip("/").split("/")
            if len(parts) != 5 or parts[0:3] != ["api", "d2pt", "hero"] or parts[4] != "pro-builds":
                return self.d2pt_error(400, "Неверный URL pro-сборки")
            hero_id = parts[3]
            url = f"{BASE}/api/hero/{quote(hero_id, safe='')}/pro-builds"
            key = f"pro-build:{hero_id}"
            try:
                data = fetch_json(url, key)
                return self.send_json(200, data)
            except HTTPError as e:
                return self.d2pt_error(502, f"D2PT pro-build HTTP {e.code}", "D2PT may return a Cloudflare challenge from datacenter IPs.")
            except URLError as e:
                return self.d2pt_error(502, "Не удалось подключиться к Dota2ProTracker", str(e.reason))
            except Exception as e:
                return self.d2pt_error(500, "Ошибка загрузки pro-сборки", str(e))
...
if __name__ == "__main__":
    ...
    print("D2PT pro-builds: /api/d2pt/hero/<id>/pro-builds")
